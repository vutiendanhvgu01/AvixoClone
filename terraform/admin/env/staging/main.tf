terraform {
  ...
  backend "s3" {
    ...
    key            = "app/staging/avixo2-admin.tfstate"
...
module "avixo2-admin" {
  ...
  vpc_id                     = "vpc-04b878c2d1b25acfa"
  family                     = "avixo2-admin"
  secret_name                = "web/avixo2-admin/staging"
  kms_key_alias              = "alias/avixo2-admin-staging"
  ...
  domain_name                = "staging.admin.avixo.co"
  ...
  alb_healthcheck_path       = "/api/hello"
  ...
  app_version                = "1.0"
  ecs_cluster_name           = "staging-cluster" # Refer to ECS cluster name
  network_mode               = "bridge"
