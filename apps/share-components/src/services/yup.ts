...
/*
  Validate NRIC
  Usage: const schema = yup.object({
    nric: yup.string().isValidNRIC(),
  });
*/

declare module 'yup' {
  interface StringSchema {
    isValidNRIC(message?: string): StringSchema;
  }
}

yup.addMethod<yup.StringSchema>(yup.string, 'isValidNRIC', function isValidNRIC(message = 'Please enter a valid NRIC') {
  return this.test({
    name: 'isValidNRIC',
    message,
    test: (nric?: string) => {
