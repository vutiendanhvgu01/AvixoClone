...
function getElementNodesInSelection(selection: RangeSelection) {
  ...
  return new Set(
    nodesInSelection.map((n: LexicalNode | null | undefined) => ($isElementNode(n) ? n : n?.getParentOrThrow())),
  );
}
...
export default function ListMaxIndentLevelPlugin({ maxDepth }: any) {
  ...
  useEffect(
    () =>
      editor.registerCommand(INDENT_CONTENT_COMMAND, () => !isIndentPermitted(maxDepth ?? 7), COMMAND_PRIORITY_HIGH),
    [editor, maxDepth],
  );
